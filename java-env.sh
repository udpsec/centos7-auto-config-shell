...
rm -f -r  apache-maven-3.6.0-bin.tar.gz

mv  apache-maven-3.6.0 /java/apache-maven-3.6.0
...
echo 'export MAVEM_HOME=/java/apache-maven-3.6.0' >> /etc/profile

echo 'export PATH=${MAVEM_HOME}/bin:$PATH' >> /etc/profile

if [ -e "/java/apache-maven-3.6.0/conf/settings.xml" ]; then

  rm -f -r  /java/apache-maven-3.6.0/conf/settings.xml
  
  mv settings.xml /java/apache-maven-3.6.0/conf/
fi
...
rm -f -r apache-tomcat-8.5.39.tar.gz

mv  apache-tomcat-8.5.39 /java/apache-tomcat-8.5
